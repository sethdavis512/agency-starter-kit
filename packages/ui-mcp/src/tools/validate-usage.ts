...
function findBgWhiteBlack(snippet: string): RegExpExecArray[] {
  ...
  const pattern = /(?<![\w-])(?:[a-z:]+:)?bg-(white|black)(?![\w-])/g;
  let match: RegExpExecArray | null;
  while ((match = pattern.exec(snippet)) !== null) {
    results.push(match);
  }
...
export function createValidateUsageHandler(
  ...
  return async (params: ValidateUsageInput) => {
    ...
    for (const m of findRawTailwindColors(snippet)) {
      ...
      const suggested = suggestTokenForRawColor(m.color, m.utility, tokens);
      violations.push({
        line,
        column,
        severity: "error",
    ...
    for (const m of findUiImports(snippet)) {
      ...
      if (importPath === "@repo/ui") {
        violations.push({
          ...
          suggestion: "e.g. `import { Button } from '@repo/ui/button'`",
          match: importPath,
        });
        continue;
      }
