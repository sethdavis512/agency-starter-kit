...
>(({ className, ...props }, ref) => (
    <Slider.Track
        ref={ref}
        className={cn(
            'relative h-1.5 w-full grow overflow-hidden rounded-full bg-neutral/15',
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <Slider.Thumb
        ref={ref}
        className={cn(
            'block h-4 w-4 rounded-full border-2 border-primary bg-surface shadow-sm transition-[transform,colors] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 data-dragging:scale-110 data-disabled:pointer-events-none data-disabled:opacity-50',
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <Slider.Value
        ref={ref}
        className={cn('text-sm font-medium tabular-nums text-neutral', className)}
        {...props}
    />
));
