    ...
    <Card className={cn("w-72", className)} {...props}>
      <CardHeader className="items-center text-center gap-3 pb-3">
        ...
        <div className="space-y-1">
          <h3 className="text-base font-semibold text-neutral">{name}</h3>
          {role && <p className="text-sm text-neutral/60">{role}</p>}
          {badge && (
      ...
      {bio && (
        <>
          <SeparatorRoot />
          <CardContent className="pt-4 text-sm text-neutral/70 text-center">
            {bio}
          </CardContent>
        </>
      )}
