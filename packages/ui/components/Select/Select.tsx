...
>(({ className, ...props }, ref) => (
    <BaseSelect.Trigger
        ref={ref}
        className={cn(
            'flex h-10 min-w-36 items-center justify-between gap-2 rounded-md border border-neutral/25 bg-surface px-3 text-sm text-neutral transition-colors select-none hover:not-data-disabled:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 data-popup-open:bg-muted data-disabled:cursor-not-allowed data-disabled:opacity-50',
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <BaseSelect.Value
        ref={ref}
        className={cn('data-placeholder:text-neutral/40', className)}
        {...props}
    />
));
...
>(({ className, children, ...props }, ref) => (
    <BaseSelect.Icon
        ref={ref}
        className={cn('flex text-neutral/50', className)}
        {...props}
    >
...
>(({ className, ...props }, ref) => (
    <BaseSelect.GroupLabel
        ref={ref}
        className={cn(
            'px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-neutral/50',
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <BaseSelect.ScrollUpArrow
        ref={ref}
        className={cn(
            'top-0 z-[1] flex h-4 w-full cursor-default items-center justify-center text-neutral/50 before:absolute before:inset-x-0 before:h-full before:content-[""]',
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <BaseSelect.ScrollDownArrow
        ref={ref}
        className={cn(
            'bottom-0 z-[1] flex h-4 w-full cursor-default items-center justify-center text-neutral/50 before:absolute before:inset-x-0 before:h-full before:content-[""]',
            className
        )}
        {...props}
    />
