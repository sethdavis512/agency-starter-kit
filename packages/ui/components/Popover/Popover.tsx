...
>(({ className, ...props }, ref) => (
    <BasePopover.Trigger
        ref={ref}
        className={cn(
            'inline-flex h-9 items-center justify-center rounded-md border border-neutral/25 bg-surface px-4 text-sm font-medium text-neutral transition-colors select-none hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 data-popup-open:bg-muted',
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <BasePopover.Popup
        ref={ref}
        className={cn(
            'origin-[var(--transform-origin)] rounded-lg border border-neutral/15 bg-surface p-4 text-neutral shadow-lg transition-[transform,opacity] duration-150 data-ending-style:scale-95 data-ending-style:opacity-0 data-instant:transition-none data-starting-style:scale-95 data-starting-style:opacity-0',
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <BasePopover.Title
        ref={ref}
        className={cn('mb-1 text-base font-semibold text-neutral', className)}
        {...props}
    />
));
