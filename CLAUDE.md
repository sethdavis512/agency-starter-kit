...
- Routes `sign-in`, `sign-up`, `sign-out`, `forgot-password`, `reset-password`, and the catch-all `api/auth/*` handler wire the flow. They are identical across portal and admin except that `admin` has no `sign-up` route (registration is closed; use `cli user:create --role admin`).
- `site-layout.tsx` (the outermost layout) runs `populateSession` middleware, which fetches the session once per request and stores it in `userContext`. Every descendant loader — `landing`, `sign-in`, `sign-up`, `forgot-password`, `reset-password`, and everything under `protected-layout` — reads the user via `context.get(userContext)` instead of calling `auth.api.getSession` again.
- **Password reset.** `forgot-password` renders `ForgotPasswordForm` (`@repo/ui/forgot-password-form`), which calls `authClient.requestPasswordReset({ email, redirectTo: "/reset-password" })`. Better Auth emails a link to its own `/api/auth/reset-password/<token>?callbackURL=/reset-password` endpoint, which redirects to the app's `reset-password` route with `?token=` (valid) or `?error=INVALID_TOKEN` (expired/unknown). That route's loader reads both query params and `ResetPasswordForm` (`@repo/ui/reset-password-form`) renders the form, the invalid-link state (with a link back to `forgot-password`), or the success state; it submits via `authClient.resetPassword({ newPassword, token })`. Tokens expire after one hour (Better Auth default).
- **Email transport** lives in `packages/auth/src/email.server.ts` and is chosen at boot by `createEmailTransport(process.env)`: with `RESEND_API_KEY` set it sends through Resend from `EMAIL_FROM` (boot throws if `EMAIL_FROM` is missing); with it unset it uses a console transport that prints each message and its link to stdout, so the reset flow works locally with no provider. When `EMAIL_OUTBOX_FILE` is also set, the console transport mirrors each message as a JSON line to that file — `playwright.config.ts` sets it (under `os.tmpdir()`) so the portal e2e can read the emailed link via `e2e/helpers/email.ts`. The module has no Prisma/Better Auth imports and is unit-tested in `email.server.test.ts`.
- **Email verification** is off by default. `REQUIRE_EMAIL_VERIFICATION=1` turns on Better Auth's `requireEmailVerification`: sign-up then creates the user but no session (the `SignUpForm` shows a "check your inbox" notice when the response `token` is null) and emails a verification link (`/api/auth/verify-email?token=…&callbackURL=/dashboard`, `autoSignInAfterVerification` is on); sign-in from an unverified account re-sends the link and fails with `EMAIL_NOT_VERIFIED`, which `SignInForm` turns into a friendlier message. `sendVerificationEmail` is always configured, so flipping the flag needs no code change.
