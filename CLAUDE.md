...
- `@repo/utils` — Small shared helpers. Subpaths: `./` (re-exports `tiny-invariant`), `./icons` (re-exports `lucide-react`), `./date` (date formatting), `./brand` (the `BRAND_NAME` placeholder-branding constant — see [Rebranding / placeholder content](./README.md#rebranding--placeholder-content) in the README), `./logger` (dependency-free JSON logger), `./security-headers` (CSP + security header builder), `./nonce` (React context carrying the per-request CSP nonce). See [Security headers and logging](#security-headers-and-logging).
...
Both apps ship an identical `app/entry.server.tsx` (scaffold-pair convention: keep them byte-for-byte the same). It does three things on every **document** response — resource routes (`/health`, `api/auth/*`) and `.data` requests return their own `Response` and never pass through it:

1. **Security headers** via `applySecurityHeaders` from `@repo/utils/security-headers`:

   | Header                      | Value                                                                                                                                                            |
   | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | `Content-Security-Policy`   | see below                                                                                                                                                        |
   | `Strict-Transport-Security` | `max-age=31536000; includeSubDomains` (always — Railway terminates TLS at the edge, so the app cannot see the public scheme; browsers ignore it over plain http) |
   | `X-Content-Type-Options`    | `nosniff`                                                                                                                                                        |
   | `Referrer-Policy`           | `strict-origin-when-cross-origin`                                                                                                                                |
   | `X-Frame-Options`           | `DENY`                                                                                                                                                           |
   | `Permissions-Policy`        | `camera=(), microphone=(), geolocation=()`                                                                                                                       |

   The CSP is a starter policy sized to what the apps load today: `default-src 'self'; script-src 'self' 'nonce-<per-request>'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; img-src 'self' data: blob: https:; connect-src 'self'; frame-ancestors 'none'; object-src 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests`. In dev (`import.meta.env.DEV`) `connect-src` also allows `ws:`/`wss:` for Vite HMR and `upgrade-insecure-requests` is dropped so plain-http localhost works. **Inter stays on Google Fonts** (not self-hosted), which is why the two `fonts.*` origins are allowed; `'unsafe-inline'` in `style-src` covers Vite's dev `<style>` injection and Base UI's inline positioning styles. Adding a third-party script, image host, or API origin means extending `buildContentSecurityPolicy` (unit tested in `packages/utils/security-headers.test.ts`), not loosening `script-src`.

2. **Per-request CSP nonce**: `entry.server.tsx` calls `createNonce()`, puts it in the header, and provides it through `NonceContext` from `@repo/utils/nonce`; `root.tsx`'s `Layout` reads it with `useNonce()` and passes it to `<ScrollRestoration nonce>` and `<Scripts nonce>`, which stamps React Router's inline hydration/route-module scripts (and the dev HMR runtime import). The same nonce goes to `<ServerRouter nonce>` and `renderToPipeableStream({ nonce })` so streamed Suspense scripts pass too. Never add an inline `<script>` without `nonce={useNonce()}` — the CSP will silently block it.

3. **Structured logging**: `handleError` in `entry.server.tsx` logs every unhandled loader/action/render error as **one JSON line** with `logger.error("Unhandled request error", { method, url, err })` from `@repo/utils/logger` (a minimal dependency-free logger: `debug/info/warn/error`, `child(fields)`, `Error` values serialized to `{ name, message, stack, cause }`, warn/error to stderr, the rest to stdout). Thrown `Response`s (the 403/404 pages) are expected control flow and are not logged. `LOG_LEVEL` (`debug | info | warn | error | silent`, default `info`) sets the threshold; both app `.env.example` files document it.

**Error boundary.** The root `ErrorBoundary` in both apps renders a `Container` → `Card` → `Button` "Back to home" page from `@repo/ui` and is identical except that `admin` first returns `ForbiddenPage` for a 403 from `requireAdmin`. A 404 renders the heading `404` (the admin e2e smoke test asserts it); other route responses render `Error <status>`; unexpected errors render "Something went wrong". `error.message` and the stack trace are shown only when `import.meta.env.DEV` — production users see the generic copy while the real error is in the logs.

**Error reporting (Sentry) hook.** No SDK is installed. `SENTRY_DSN` is reserved in both `.env.example` files, and `handleError` in `entry.server.tsx` carries a commented hook point. To wire it: install `@sentry/react-router` in both apps, initialise it once at module scope from `process.env.SENTRY_DSN`, and call `Sentry.captureException(error, { extra: { url: request.url } })` at the hook — keep the JSON log line as well.
